...
// searchInputFunc = function (searchInputValue) {
//   if (searchInputValue != "") {
...
document.querySelector(".WP-SearchBtn").addEventListener("click", () => {
  ...
  if (input) {
    // Ищем элементы на странице, содержащие введённый текст
    const elements = Array.from(document.querySelectorAll("body *")); // Все элементы страницы
    const foundElement = elements.find(
      (el) => el.innerText?.toLowerCase().includes(input) // Проверяем, есть ли текст в элементе
    );

    if (foundElement) {
      // Подсвечиваем найденный элемент (например, временно)
      foundElement.style.outline = "2px solid red"; // Визуальное выделение
